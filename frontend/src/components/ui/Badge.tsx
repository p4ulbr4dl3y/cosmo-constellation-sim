...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const variantStyles: Record<NonNullable<BadgeProps['variant']>, string> = {
    ...
    accent: 'bg-sky-500/15 text-sky-200 border-sky-500/30',
    warning: 'bg-amber-500/15 text-amber-300 border-amber-500/30',
  }

  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-mono font-medium border ${variantStyles[variant]} ${className}`}
      {...props}
    >
      {children}
    </span>
