...
def validate_scenario(s: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(env, dict):
        ...
        if horizon_s is not None and (not isinstance(horizon_s, int) or isinstance(horizon_s, bool)):
            errors.append("Горизонт расчета horizon_s должен быть целым положительным числом секунд.")
            valid_time_types = False
    ...
    if not isinstance(design, dict):
        ...
        if not isinstance(stage, int) or isinstance(stage, bool) or stage not in (1, 2, 3):
            errors.append(f"Параметр launch_stage должен быть целым числом 1, 2 или 3. Получено: {stage}")
        ...
        if not isinstance(sats, list) or not sats:
            ...
            for idx, sat in enumerate(sats):
                ...
                if not is_finite_number(slot):
                    errors.append(f"Спутник '{sid}' имеет некорректный slot_deg={slot}. Ожидается число.")
    ...
    if not isinstance(ground, list) or not ground:
        ...
        if not has_client:
            errors.append("В сценарии должен присутствовать хотя бы один клиентский пункт (role='client').")
        if not has_gateway:
    ...
    if not isinstance(gw_outages, list):
        ...
        for idx, f in enumerate(gw_outages):
            ...
            end_s = f.get("end_s")
            if not is_finite_number(start_s) or not is_finite_number(end_s):
                errors.append(f"Интервал недоступности шлюза #{idx} должен содержать числовые start_s и end_s.")
            elif not (0 <= start_s < end_s <= max_h):
                errors.append(
