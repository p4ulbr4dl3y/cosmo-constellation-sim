...
def get_health() -> dict[str, str]:
    return {"status": "ok", "service": "satellite-constellation-backend", "version": "0.1.0"}


@router.get("/presets", response_model=list[PresetSummary], summary="List available scenario presets")
...
def compare(req: CompareRequest) -> dict[str, Any]:
    ...
    if errors_a:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail={"message": "Сценарий A содержит ошибки валидации", "errors": errors_a},
        )
    ...
    if errors_b:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail={"message": "Сценарий B содержит ошибки валидации", "errors": errors_b},
        )
